...
function openWeatherToSimpleWeather(weatherId) {
  if(isStormy(weatherId)) {
    return simpleWeatherTypes[7];
  } else if(weatherId >= 300 && weatherId <= 500){
    return simpleWeatherTypes[4];
  } else if(weatherId === 501 || weatherId === 502) {
    return simpleWeatherTypes[5];
  } else if(weatherId > 502 && weatherId <= 522){
    return simpleWeatherTypes[6];
  } else if(weatherId >= 600 && weatherId <= 621)   {
    return simpleWeatherTypes[8];
  } else if(weatherId >= 701 && weatherId <= 741)   {
    return simpleWeatherTypes[9];
  } else if(weatherId === 800)                      {
    return simpleWeatherTypes[1];
  } else if(weatherId === 801 || weatherId === 802) {
    return simpleWeatherTypes[2];
  } else if(weatherId === 803 || weatherId === 804) {
    return simpleWeatherTypes[3];
  }
  return 0; // 0 => 'error'
}

function isStormy(weatherId) {
...
module.exports = {
  weather: function(req, res) {
    var ts = parseInt(req.param('timestamp'), 10) || Date.now();
    ...
    if (cached) {
      //cache hit
      closest = closestForecast(cached, ts);
    } else {
